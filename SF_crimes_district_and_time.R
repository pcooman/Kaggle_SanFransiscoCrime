...
crimes_by_district <- table(data$PdDistrict,data$Hour,data$Category)
...
names(crimes_by_district_melt) <- c("PdDistrict","Hour","Category","Prob")
crimes_by_district_norm <- data.frame(crimes_by_district_melt)
crimes_by_district_norm2 <- acast(crimes_by_district_norm, PdDistrict + Hour ~ Category, crimes_by_district_norm = "Prob")
...
colnames(crimes_by_district_norm2)[2] <- "Hour"
...
submission <- subset(test, select = c("Id","PdDistrict","Hour"))
# Merge the test data with the normalized crimes_by_district
submission <- merge(submission,crimes_by_district_norm2, by = c("PdDistrict","Hour"))
# Drop the PdDistrict column
submission$PdDistrict <- NULL
submission$Hour <- NULL
...
write_csv(submission, "Based_on_PdDistrict_and_Hour.csv")
file.info("Based_on_PdDistrict_and_Hour.csv")$size
dir()